...
A = 4;
rho = 0.95;
lambdas=[0.0001,0.0003,0.001,0.003,0.01,0.03,0.1,0.3,0.99];
quantnum=10000;
cp=[0,1,2,3,4];
ci=[0,0.02,0.06,0.2];
...
for idxlambda = 1:length(lambdas)
    lambda=lambdas(idxlambda);
...
%%
    ...
    V_Oracle=rho*(Alpha*cp')/(1-rho)+((Beta{A}-Alpha)*cp'+ci(A))*(rho/(1-rho)+(lambda-1)*rho/(1-rho*(1-lambda)));
    Reg_approx(idxlambda)=V_approx(1,1)-V_Oracle;
    Reg_prop(idxlambda)=V_prop(1,1)-V_Oracle;
    Reg_QCD(idxlambda)=V_QCD(1,1)-V_Oracle;
    Reg_DQCD(idxlambda)=V_DQCD(1,1)-V_Oracle;
    Reg_Upper(idxlambda)=V_Upper(1,1)-V_Oracle;
    RegProportion1(idxlambda)=(Reg_QCD(idxlambda)-Reg_prop(idxlambda))/Reg_QCD(idxlambda);
    RegProportion1Up(idxlambda)=(Reg_QCD(idxlambda)-Reg_Upper(idxlambda))/Reg_QCD(idxlambda);
    RegProportion2(idxlambda)=(Reg_DQCD(idxlambda)-Reg_prop(idxlambda))/Reg_DQCD(idxlambda);
    RegProportion2Up(idxlambda)=(Reg_DQCD(idxlambda)-Reg_Upper(idxlambda))/Reg_DQCD(idxlambda);
end
%%

figure;
semilogx(lambdas,Reg_prop,'Linewidth',1.4,'Marker','o');
hold on;
grid on;
semilogx(lambdas,Reg_Upper,'Linewidth',1.4,'Marker','^');
semilogx(lambdas,Reg_QCD,'Linewidth',1.4,'Marker','*');
semilogx(lambdas,Reg_DQCD,'Linewidth',1.4,'Marker','s');
semilogx(lambdas,Reg_approx,'Linewidth',1.4,'Marker','d');
...
% figure;
% semilogx(lambdas,RegProportion1,'Marker','o');
% hold on 
% semilogx(lambdas,RegProportion1Up,'Marker','+');
...
% 
% figure;
% semilogx(lambdas,RegProportion2,'Marker','o');
% hold on 
% semilogx(lambdas,RegProportion2Up,'Marker','+');
